...
Every request to `api.coinversa.ai/api/public/v1/*` requires an API key. Get one at [coinversa.ai/developers](https://coinversa.ai/developers).
...
Your key's tier is set by your active subscription. Free is the default; paid tiers are managed in the [billing portal](https://coinversa.ai/developers/billing).
...
<AccordionGroup>
  <Accordion title="401 Unauthorized — missing key">
    ```json
    {
      ...
      "message": "Hey — you need an API key to use this endpoint. Get a free one at https://coinversa.ai/developers",
      "signup_url": "https://coinversa.ai/developers"
    }
    ```
  ...
  <Accordion title="401 Unauthorized — invalid key">
    ...
    ```
    Action: check the key isn't revoked or rotated in the [portal](https://coinversa.ai/developers/keys).
  </Accordion>

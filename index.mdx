...
There are three ways into the same data, all with the same API key — pick the one that matches what you're building.
...
<Columns cols={2}>
  ...
  <Card title="Need fills as they happen?" icon="bolt" href="/websocket/overview">
    Use the **WebSocket**. Every fill on Hyperliquid streamed at block cadence — the firehose, one coin, or one wallet — with the trader's wallet and PnL on each fill. Same key; limits are on connections, not messages.
...
<Columns cols={2}>
  ...
  <Card title="WebSocket API" icon="bolt" href="/websocket/overview">
    Stream every fill: auth on the upgrade, three feeds, sequence numbers, plan limits.
  </Card>
</Columns>
...
- **Indexers & live feeds** — subscribe over the [WebSocket](/websocket/overview) and receive every fill as it lands, instead of polling.

...
For fills themselves, polling is the wrong tool: the [WebSocket](/websocket/overview) pushes every fill at block cadence, with a per-subscription sequence number so you can tell when you missed one.

...
Four raw feeds today: three stream fills, the fourth streams a coin's full order book. Each is a separate subscription, and you can hold as many as your plan's per-connection cap allows on one socket. Every frame carries the source block height as `seq`.
...
- **`seq` on every data frame** — the source block height, and your reconnect cursor.

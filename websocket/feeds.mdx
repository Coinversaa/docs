...
<Note>
`tradesByUser`, `liquidationsByUser` and `leverageUpdatesByUser` accept any address. Hyperliquid's own websocket streams only wallets you control and caps subscribers at ten users per IP; there is no such restriction here beyond your plan's subscription count. The firehose answers `feed not in tier` on Free, and the liquidation, leverage and `l4Book` feeds answer it below Pro — check `connected.data.limits.feeds` for what your key may subscribe to.
</Note>
...
- **Liquidations and leverage changes are feeds of their own**, not something you filter out of fills yourself — and both take any address.

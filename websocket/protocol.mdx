...
Because a data frame carries no subscription id, key any per-stream tracking on what the frame itself identifies: the `channel` plus the coin or wallet of its first fill. Two `tradesByCoin` subscriptions on one connection both arrive on channel `tradesByCoin`; they now share the same height sequence, so tracking on the channel alone no longer produces phantom gaps, but it also cannot tell you which subscription a frame belongs to.
...
| `data` | What it means |
|---|---|
| `sequence is not retained` | The cursor is not in the buffer — often because the feed group had gone cold. |

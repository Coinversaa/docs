...
| Pro | 5 | 100 | all, including the `l4Book` [order book](/websocket/order-book), [liquidations](/websocket/feeds#liquidations) and [leverage changes](/websocket/feeds#leverage-and-margin) | 250 GB |
...
| Feed not in plan | `{"channel":"error","data":"feed not in tier"}` on the open socket — the firehose on Free, and `l4Book`, `liquidations`, `liquidationsByUser`, `leverageUpdates` and `leverageUpdatesByUser` below Pro. |

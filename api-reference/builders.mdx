...
Auth on the REST API is the `X-API-Key` header — **not** a Bearer token (the REST API ignores an `Authorization: Bearer` header and treats it as a missing key; the [WebSocket](/websocket/overview#authentication) is where a Bearer token belongs).

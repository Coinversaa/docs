...
The Coinversa Pulse REST API exposes everything the MCP server uses and more — 110+ endpoints covering trader analytics, builder analytics, market data, live risk, HIP-4 outcomes, entity resolution, and the cross-market asset registry.
...
The **WebSocket** fill stream is not part of this spec or this reference — it is a separate protocol on `wss://ws.coinversa.ai`, documented in the [WebSocket section](/websocket/overview).
...
- **Live** — near-real-time market and risk (`/live/*`). For fills pushed as they land rather than polled, use the [WebSocket](/websocket/overview).

...
- **`seq` is a monotonic source position, not a per-subscription counter** — and it is not one kind of number. It is the source block height on `trades`, `tradesByCoin`, `tradesByUser`, `liquidations`, `liquidationsByUser` and `l4Book`; it is the replica block's Unix timestamp in **nanoseconds** on `leverageUpdates` and `leverageUpdatesByUser`. Never document a client that compares the two or holds one cursor across both. Data frames carry `channel`, `seq` and `data` and **no subscription id**, so a client with several subscriptions on one channel cannot attribute a frame from the frame alone — say so plainly.
